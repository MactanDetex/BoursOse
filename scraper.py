...
# Si nécessaire, tu peux décommenter et remplir pour gérer l’authentification :
...
def scrape_forum():
    ...
    entries = []
    # Récupère les 20 premiers topics
    for link in soup.select('a[href^="/bourse/forum/1rPOSE/detail/"]')[:20]:
        ...
        author_el = parent.select_one('a.c-forum__author')
        author = author_el.get_text(strip=True) if author_el else "Forum Boursorama"
